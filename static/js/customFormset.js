...
function addForm(btn, prefix) {
    ...
    if (formCount < 1000) {
        ...
        // if( window.location.pathname.split('/')[1] == 'home-road-certifications'){
        $(row).find('.home-type').empty().append('<option value select>छान्नुहोस</option>').attr('disabled', true);
        $(row).find('.road-type').empty().append('<option value select>छान्नुहोस</option>').attr('disabled', true);
        $(row).find('.estimated-cost').val('').attr('disabled', true);
        $(row).find('.biggha').val(0);
        $(row).find('.kattha').val(0);
        $(row).find('.dhur').val(0);
        // }
